...
*)
  ...
  let rec merge s1 s2 = 
   if (s1.hd = s2.hd) then {hd = s1.hd ; tl = Susp (fun () -> merge (force s1.tl) (force s2.tl))}
   else if (s1.hd < s2.hd) then {hd = s1.hd ; tl = Susp (fun () -> merge (force s1.tl) s2)} 
   else {hd = s2.hd ; tl = Susp (fun () -> merge s1 (force s2.tl))}
