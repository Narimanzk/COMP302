 module Spreadsheet : SPREADSHEET = 
  ...
  let rec change m = match (List.hd m) with
   | [] -> []
   | _ -> (List.map List.hd m)::(change (List.map List.tl m))

  let rec compute_column m f = List.map f (change m)
  ...
  let rec update e = match e with
    | Num(n) -> ()
    | Plus(e, e1, e2) -> update ( add e1 e2 )
    | Times(e, e1, e2) -> update ( mult e1 e2 )
